...
function three_mall_show($select_id){
	$mall=C('THREE_MALL_ID');
	$mall_html='';
	$select='';
	foreach ($mall as $key => $value) {
		...
		$mall_html=$mall_html.'<option value ="'.$key.'" '.$select.'>'.$value.'</option>';
	}
	return $mall_html;
}

function broadcast_class_show($select_id){
	$mall=C('BROADCAST_CLASS');
	$mall_html='';
	$select='';
	foreach ($mall as $key => $value) {
		if($select_id==$key){
